...
top.movies.2010.2019 <- dt.movie.actor[
    year >= 2010 & year <= 2019, 
    .N, by = list(movie, votes)
    ][order(-votes)][1:50, movie]

actors.in.top <- dt.movie.actor[
    movie %in% top.movies.2010.2019, 
    unique(actor)]
...
g.movies <- bipartite.projection(g.bipartite)$proj1
summary(g.movies)
...
g.movies.connected <- induced.subgraph(
  g.movies,
